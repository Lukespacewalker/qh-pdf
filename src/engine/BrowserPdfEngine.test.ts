...
// Only isolate browser transport/rendering. The exact production worker assembly
// creates and reopens real PDFs here; Chromium exercises the actual Worker.
...
async function source(id: string, widths: number[], rotation = 0): Promise<ImportedDocument> {
  ...
  return { id, fileName: `${id}.pdf`, mimeType: 'application/pdf', kind: 'pdf', bytes: Uint8Array.from(await pdf.save()).buffer,
...
beforeEach(() => { engine = new BrowserPdfEngine(exportLocally); });
...
describe('real PDF export', () => {
  it('preserves source rotation and applies the additional workspace rotation', async () => {
    ...
    const result = await engine.exportWorkspace(new Map([[doc.id, doc]]), { pages: [page('a', doc.id), page('b', doc.id, 0, 90)], selectedPageIds: [] });
  ...
  it('interleaves documents and duplicates pages without changing their dimensions', async () => {
    const a = await source('a', [111, 222]); const b = await source('b', [333]);
    const result = await engine.exportWorkspace(new Map([[a.id, a], [b.id, b]]), { pages: [page('1', 'a', 1), page('2', 'b'), page('3', 'a'), page('4', 'a', 1)], selectedPageIds: [] });
    const pdf = await PDFDocument.load(await result.arrayBuffer());
  ...
  it('rejects a missing source instead of silently dropping a requested page', async () => {
    await expect(engine.exportWorkspace(new Map(), { pages: [page('lost', 'missing')], selectedPageIds: [] })).rejects.toMatchObject({ code: 'export-failed' });
  });
  it('rejects an empty workspace instead of creating an unexpected blank page', async () => {
    await expect(engine.exportWorkspace(new Map(), { pages: [], selectedPageIds: [] })).rejects.toMatchObject({ code: 'export-failed' });
  });
  it('rejects an out-of-range source page', async () => {
    const doc = await source('one', [200]);
    await expect(engine.exportWorkspace(new Map([[doc.id, doc]]), { pages: [page('bad', doc.id, 5)], selectedPageIds: [] })).rejects.toMatchObject({ code: 'export-failed' });
  });
  it('keeps original bytes intact across repeated exports', async () => {
    const doc = await source('original', [210, 220]); const original = new Uint8Array(doc.bytes).slice();
    const docs = new Map([[doc.id, doc]]); const workspace = { pages: [page('a', doc.id, 1)], selectedPageIds: [] };
    for (let i = 0; i < 2; i++) {
