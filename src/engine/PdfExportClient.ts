...
export function makeExportRequest(documents: ReadonlyMap<string, ImportedDocument>, workspace: WorkspaceState): ExportRequest {
  ...
  const sources = [...ids].map(id => {
    ...
    return {
      id, kind: doc.kind, mimeType: doc.mimeType,
      blob: new Blob([doc.unlockedBytes ?? doc.bytes], { type: doc.mimeType }),
...
/** One worker per operation: terminate also interrupts synchronous parsing/serialization. */
export function exportInWorker(
  documents: ReadonlyMap<string, ImportedDocument>, workspace: WorkspaceState,
      ...
      worker.onmessage = ({ data }: MessageEvent<ExportResponse>) => {
        ...
        if (data?.type === 'progress') {
          if (['assembling', 'writing'].includes(data.phase) && Number.isInteger(data.completed) && data.total === request.pages.length && data.completed >= 0 && data.completed <= data.total) {
            try { options.onProgress?.({ phase: data.phase, completed: data.completed, total: data.total }); }
