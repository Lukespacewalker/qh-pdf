...
- [x] 1. Establish design, contracts and isolated worktree. Define `CropMargins`, export numbering sections/style/watermark/compression types in domain modules; keep UI dependent on PdfEngine. Baseline Node 24 install/unit tests.
- [x] 2. Numbering/watermark engine: pure validation/formatting plus `decoratePdf(output, settings, outputIndices?)` in an engine-only module. A delegated writer owns new domain export-options/numbering and decoration files, tests, font assets/license and fontkit dependency only. Root wires assembly/protocol/UI after the commit. Tests: decimal/Roman/Latin/Thai boundaries, overlapping/out-of-range sections, actual extracted output text and rotated/cropped page positions. Real font rendered visibly.
...
- Export decoration and compression were isolated Sol assignments with Root-owned integration. No shared-file concurrent writers. Domain `41d945e`, decorator `2da6b22`, compression `b0af962` integrated; callback sequencing/real output preview wired at `577c5f1`. English/Thai foundation `eeb1696` and full workspace translation `20f3b0c` are integrated.
- Pinned QPDF 12.3.2 probe demonstrated eligible-image optimization. Presets: lossless Flate level 9/object streams; balanced adds image optimization JPEG quality 75; small uses 45. Synthetic 900×700 RGB image PDF: 1,868,484 B input → 1,868,104 / 321,207 / 193,365 B respectively. Measurements are fixture evidence, not promised savings. Some images are ineligible and remain unchanged.
- Final integrated code `2be8b45` plus the committed acceptance-test delta: Node 24 unit tests 195/195, build, Chromium browser tests 45/45, and local Cloudflare tests 46/46 passed. The new nine finishing workflows cover real selected numbering/Latin and Thai rollover/duplicate independence, advanced Roman-to-decimal sections, all source rotations and nonzero CropBoxes, all compression levels with text/crop/rotation preservation and password protection, font/compression failure/retry, cancellation with no download, image crop rotation/recovery/preview consistency, and Thai controls/errors/preference at 390 px. Privacy and deferred-runtime assertions remain enabled. An initial overlapping browser/hosting run collided in Playwright's shared artifact directory; browser tests were rerun sequentially and passed. No application assertion was relaxed.
- Thai sara-am extraction was corrected at `27ce8da`: the embedded text uses canonically compatible decomposed Thai characters with unchanged shaped glyph placement, avoiding the duplicated vowel. Tests compare NFKC-equivalent text. `a63de28` also rejects page numbers that would leave the visible area. Actual downloaded PDFs were rendered with Poppler and inspected at all four source rotations: Thai watermark, Roman/decimal sequence, upright decoration and bottom-center placement are correct.
- Independent fresh-session reviewer inspected frozen `7bb0df1` and executed crop/keyboard/390 px flows without a new confirmed blocker. Final frozen delta review and PR CI remain pending. Cross-browser execution and deployment remain out of scope.
