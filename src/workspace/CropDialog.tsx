...
export function CropDialog({ page, document, count, engine, opener, onApply, onClose }: {
  ...
  return <dialog ref={dialog} className="finishing-dialog crop-dialog" aria-labelledby="crop-title"
    ...
    <div className="finishing-heading"><h2 id="crop-title">{t('Crop selected pages')}</h2>
      <button className="btn" onClick={() => dialog.current?.close()} autoFocus>{t('Cancel')}</button></div>
    <p>{t('Remove a percentage from each edge. The same crop applies to all {count} selected pages.', { count })}</p>
    <div className="crop-preview" aria-label={t('Crop preview')}>
      {image ? <div className="crop-image"><img src={image} alt={t('First selected page before cropping')} />
        {valid && <div className="crop-kept" aria-label={t('Area to keep')} style={{
          top: `${crop.top * 100}%`, right: `${crop.right * 100}%`, bottom: `${crop.bottom * 100}%`, left: `${crop.left * 100}%`,
        }} />}</div> : failed ? <div role="alert">{t('Preview unavailable.')} <button className="btn" onClick={() => setRetry(value => value + 1)}>{t('Retry preview')}</button></div> : <p role="status">{t('Rendering page…')}</p>}
    </div>
    <div className="finishing-fields crop-fields">{(['top', 'right', 'bottom', 'left'] as const).map(side => <label className="field" key={side}>
      {t(({ top: 'Top (%)', right: 'Right (%)', bottom: 'Bottom (%)', left: 'Left (%)' })[side])}
      <input type="number" min="0" max="99.9" step="0.1" value={margins[side]} onChange={event => setMargins({ ...margins, [side]: event.target.value })} />
    </label>)}</div>
    {!valid && <p className="notice" role="alert">{t('Crop margins must leave a visible area on the page.')}</p>}
    <p className="sub">{t('Cropping hides content outside the edges. It does not permanently erase it.')}</p>
    <div className="dialog-actions">
      <button className="btn" onClick={() => setMargins({ top: '0', right: '0', bottom: '0', left: '0' })}>{t('Reset crop')}</button>
      <button className="btn primary" disabled={!valid} onClick={() => { onApply(crop); dialog.current?.close(); }}>{t('Apply to {count} pages', { count })}</button>
    </div>
  </dialog>;
}
